...
public class ConstraintsHelper {
    ...
    private boolean checkConstraints() {
        ...
        try {
            ...
            // Check if a specific package is enabled
            String rPackageEnb = a.getString(R.styleable.aicp_SelfRemovingPreference_requiresPackageAvailable);
            if (rPackageEnb != null) {
                boolean negated = isNegated(rPackageEnb);
                if (negated) {
                    rPackageEnb = rPackageAvl.substring(1);
                }
                boolean available = AicpUtils.isPackageAvailable(rPackageEnb, mContext);
                if (available == negated) {
                    return false;
                }
            }
