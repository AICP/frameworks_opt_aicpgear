...
public class ConstraintsHelper {
    ...
    private boolean checkConstraints() {
        ...
        try {
            ...
            String rPackage = a.getString(R.styleable.aicp_SelfRemovingPreference_requiresPackageInstalled);
            if (rPackage != null) {
                boolean negated = isNegated(rPackage);
                if (negated) {
                    rPackage = rPackage.substring(1);
                }
                boolean available = AicpUtils.isPackageInstalled(mContext, rPackage);
            ...
            String rPackage = a.getString(R.styleable.aicp_SelfRemovingPreference_requiresPackageAvailable);
            if (rPackage != null) {
                boolean negated = isNegated(rPackage);
                if (negated) {
                    rPackage = rPackage.substring(1);
                }
                boolean available = AicpUtils.isPackageAvailable(rPackage, mContext);
                if (available == negated) {
                    return false;
                }
            }
